...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **It is telling me to use spec() to retrieve column specification, and it also asked me to specify column types.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "district" field? What data type is the "precinct" field? What data type is the "mail" column? Answer below. **District is dbl, i.e. quantitative/numbers, while the mail column is the same.**
...
**Task** Run the following codeblock to group by mail votes, count the number of precinct results, then sort from highest to lowest on count. How many results are there where mail is NA? What's the implication there? Answer below. **There are 402,345 results where mail is NA. With that large of a number, implies mail-in votes weren't kept track of in MANY cases.**:
...
**Task** Run the following codeblock to find the number of registered voters grouped by voter status reason, using summarise() to count. Are any of these worthy of exploration to you? Why or why not? Answer below. **I think the large number of confirmation not confirmed is definitely something that jumps out as worthy of exploration.**
...
**There are 14 rows in this data set.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "amount" field? Answer below. **The amount field is character/text.**
...
**I'm not sure what it does here; have been looking at data and have yet to find the difference?> **
...
**Task** Run the following codeblock to attempt to add up the amount of all contributions. What does the error say when you run this code? What do you think the error means? Answer below. **It says that the sum() is an invalid type. I think it's because sum is labeled as a character instead of a number.**
...
**Task** Run the following codeblock to attempt to change the datatype for the amount field to a number. What is the new data type (three letter code) for amount? Answer below. **The new type for amount is <dbl>**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many are there? What is the donor name? Answer below. **Just one duplicate with the name Derrick Hamilton**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. Add a description of what this code does to your reference notebook. **It has 13 rows now.**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, standardizing the field. Look at the difference in the result - what changed? **It eliminated the longform zipcode, limiting it to six characters.**
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field. How many mispellings of Conowingo remain after running this code? Answer below. **1 misspelling**
...
**Task** Run the following codeblock to use case_when() to fix misspellings of Conowingo in the "city" field using both the exact match method and the str_detect() method. How many mispellings of Conowingo remain after running this code? Answer below. Add a description of what this code does to your reference notebook. **There are now no misspellings**
