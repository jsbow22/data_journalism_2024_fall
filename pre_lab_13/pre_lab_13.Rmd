...
```{r}
create_credentials(GROQ_API_KEY = "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")
```
...
**I honestly can't tell the difference between the first ten names and the second ten. The first ten seem to be a little bit more creative/exotic? It's hard to tell.**
...
**It seems to have extracted the data, but I can't find the csv file. Not sure if I'm doing something wrong. However, in the thing below, it does seem that the correct data points were extracted by AI.**
...
**I think there's one thing that is slightly wrong, with it noting the portfolio response as only being the thing highlighted (budget, financial services, etc.). I would probably take the portfolio out, as that seems to be part of the job description and not its own data column.**
...
**This does make sense, as if it can't find the function it may mean that you haven't updated (according to the AI response). This makes sense to me with my knowledge.**
...
**I think the biggest assumption is that it is assuming column names when i haven't given them, at least when it is suggesting code to write.**
...
**It is mostly pretty good, but the biggest issue is that it did not understand the American flag-gloved hand at the bottom. It thought that it was some kind of wing (it is not).**
...
**Despite following the code exactly, I am getting an error message on this question. It says that HTTP 404 cannot be found.** 
