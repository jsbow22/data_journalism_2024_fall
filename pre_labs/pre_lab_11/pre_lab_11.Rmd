...
**Task** Run the following code to extract the table we want, clean up the names and remove the total row. Look at the rename() function - what do you think the numbers 2 through 6 are referring to? **The numbers seem to be referring to the column in which that candidate's data is situated.**
...
**Task** Run the following code in the gray-colored codeblock below to find the text inside <li> tags. Describe the output: **It semeed to clean it up so that it's easy to see what each committee/line was instead of being a huge mess of text and data.**
...
### Task 11: Turn the contents of the <li> and <a> tags into a dataframe **Task** Run the following code in the gray-colored codeblock below to make a dataframe. Describe the output: **It created committees, the list of the 55 committees (in the side window as "list of 55") and then also created a committees with URLs with 55 rows.**
