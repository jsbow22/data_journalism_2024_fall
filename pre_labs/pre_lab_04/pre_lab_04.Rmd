...
**Task** Run the following code to group and count records in the recipient_name column, and filter to examine only recipients that start with W. Name a problem that you see that is preventing proper grouping. **Different companies have multiple entries, both under the same name and incorrect spellings or abbreviations of the name.**
...
**Task** Run the following code to group by recipient_clean and recipient_name, then filter for cities that start with W. Name three changes the refinr function made in the first 20 results. **Grouped similar names together; i.e. all W. MILLAR AND CO (or any variation with caps) are the same. Did the same with Walmart.**
...
**Task** Run the following code to use the n_gram_merge() function to clean the data, then group by city_clean and city to examine cities that start with A. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **Looks like things are a little more standardized? Still slightly confused by results.**
