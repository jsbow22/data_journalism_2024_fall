...
**Task** Run the following code to load data. Describe what you see in the answer space below. What columns exist in this data? **There are columns that explain location, such as precinct data, school, council, and more. Then there's Congress,, polling ID, and shape data.**
...
**Task** Run the following code. Describe the output in the space below: what kind of information does it contain? **It's a data table that describes precincts in PG County. It includes details like Congressional district, but also shape data. All is character data except for the shapes.**
...
**Task** Run the following code. Describe the output in the space below. **It produces a map of the precincts with latitude and longitude.**
...
**Task** Run the following code. Describe the output in the space below. **The code below says that, to enable caching of data, to set options to options(tigris_use_cache = TRUE).**
...
**Task** Run the following code. Describe the output in the space below. **It is an 18 column, 3.235 row data set that includes location data, names, and other categories. There are also coilumns to detrmine the shape/map output.**
...
**Task** Run the following code. Describe the output in the space below. **I believe it just filtered the data to make it only Maryland counties.**
...
**Task** Run the following code. Describe the output in the space below. **It is a map of Maryland counties (outlined by borders on the state map) with latitude and longitude on the x and y axes.**
...
**Task** Run the following code. Describe the output in the space below. **This is pulling the PG county map and adding on the precincts. It also establishes a minimal theme.**
...
**Task** Run the following code. Describe the output in the space below. **It is a dataset of voter turnout in PG county, broken down by precinct and voter types.**
...
**Task** Run the following code. Describe the output in the space below. **It joined the precinct data with the turnout data, allowing us to see turnout by precinct in the same data set.**
...
**Task** Run the following code. Describe the output in the space below. **It has now colored the different precincts with regard to the total eligible turnout.**
...
**Task** Run the following code. Describe the output in the space below. **It shows voter data by each county in maryland (namely by party).**
...
**Task** Run the following code. Describe the output in the space below. **Gives a population for each county in Maryland using the census data.**
...
**Task** Run the following code. Describe the output in the space below. **Gives the counties with population, but also with geometry data.**
...
**Task** Run the following code. Describe the output in the space below. **It displays the county populations and voters by county in two separate outputs.**
...
**Task** Run the following code. Describe the output in the space below. **It has joined the two data sets with some fixes/cleans, leaving us with an organized table with the geometry data in there.**
...
**Task** Run the following code. Describe the output in the space below. **It has now joined the data, putting it one table.**
...
**Task** Run the following code. Describe the output in the space below. **It fixed the NA categories.**
...
**Task** Run the following code. Describe the output in the space below. **It now has the counties, their voter totals, and then the voters per 10k of population added in.**
...
**Task** Run the following code. Describe the output in the space below. **It has added color to the graph, which is meant to show the number of voters per 10,000 residents in each county. I think its odd that the darker is lower, can't really see.**
...
**Task** Run the following code. Describe the output in the space below. **It has now given a different set of colors to delineate the voters per 10k**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see? **It seems like the western and southern parts of the state vote less, while the further east you go there is more voting? Southeast also has high turnout.**
...
**Task** Run the following code. Describe the output in the space below. **Gives the number of unaffiliated voters out of every 10k.**
...
**Task** Run the following code. Describe the output in the space below. What regional patterns do you see, especially on the ends of the scale? **There are far more unaffiliated voters in the center of the state. It seems like the least are more scattered.**
